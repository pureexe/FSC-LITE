...
Func _ExpListView_Create($hWnd, $sStartDir, $iColumns, $bShowHidden = False, $iX = 1, $iY = 1, $iWidth = 150, $iHeight = 150, $iStyle = 0x0000000D, $iExStyle = 0x00000000, $fCoInit = False)
	...
	$aListViews[$aListViews[0][0]][5] = '100;100;100;100;100'
...
Func _SetColumnsWidths($iArrayIndex, $hWnd)
	...
	If BitAND($aListViews[$iArrayIndex][3], 1) Then _GUICtrlListView_AddColumn($hWnd, 'ขนาด', Int($aColumnWidths[1]))
	If BitAND($aListViews[$iArrayIndex][3], 2) Then _GUICtrlListView_AddColumn($hWnd, 'แก้ไขครั้งสุดท้ายเมื่อ', Int($aColumnWidths[2]))
	If BitAND($aListViews[$iArrayIndex][3], 4) Then _GUICtrlListView_AddColumn($hWnd, 'ประเภทไฟล์', Int($aColumnWidths[3]))
	If BitAND($aListViews[$iArrayIndex][3], 8) Then _GUICtrlListView_AddColumn($hWnd, 'สร้างเมื่อ', Int($aColumnWidths[4]))
	If BitAND($aListViews[$iArrayIndex][3], 16) Then _GUICtrlListView_AddColumn($hWnd, 'เปิดใช้งานครั้งสุดท้ายเมื่อ', Int($aColumnWidths[5]))
	...
	Select
		...
		Case Else
			...
			Else
				$sChangeDir = $aListViews[$iArrayIndex][1] & '\' & $sItemText
			EndIf
			if  FileGetAttrib($sChangeDir)="A" Or FileGetAttrib($sChangeDir)="AH"  Then ShellExecute($sChangeDir)
